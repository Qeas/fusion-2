...
echo "$(dirname "$0")"
fusionImage="nexenta/fusion:2.0.3"
...
EOF
        ...
        while [ $correct -eq 0 ]; do
            ...
            if [[ "$staticip" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}(\/([0-9]|[1-2][0-9]|3[0-2]))$ ]]; then
                correct=1;
            else
        ...
        while [ $correct -eq 0 ]; do
            ...
            if [[ "$gatewayip" =~ ^([0-9]{1,3}\.){3}[0-9]{1,3}$ ]]; then
                correct=1;
            else
        ...
        while [ $correct -eq 0 ]; do
            ...
            if [[ "$nameservers" =~ ([0-9]{1,3}\.){3}[0-9]{1,3}((,\s*\([0-9]{1,3}\.){3}[0-9]{1,3})? ]]; then
                correct=1;
            else
...
if [ $selectedOptIndex = 1 ]; then
    ...
    echoBlue "Do you want to use DHCP of static IP?"
    echo "----------------"
    opts=("DHCP" "Static")
    displayNetworkOptions
    readSelectedOption opts
